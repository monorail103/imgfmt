...
async fn fetch(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    // ルートを定義
    router
        .get_async("/", serve_ui)  // ルートページでUIを提供
        .post_async("/convert", handle_image_conversion)
        .run(req, env).await
}

// UIを提供する関数
async fn serve_ui(_req: Request, _ctx: RouteContext<()>) -> Result<Response> {
    let html = r#"
<!DOCTYPE html>
<html lang="ja">
<head>
...
async fn handle_image_conversion(mut req: Request, _ctx: RouteContext<()>) -> Result<Response> {
    // ...existing code...
    let url = req.url()?;
    ...
    let output: ImageOutput = match image_processor::process_image(&image_bytes, &params) {
        Ok(output) => output,
